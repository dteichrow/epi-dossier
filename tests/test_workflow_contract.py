...
def test_newsdesk_schedule_uses_one_authoritative_twice_hourly_publish_stream() -> None:
    workflow = load_workflow(".github/workflows/newsdesk-public-publish.yml")
    schedule = workflow["on"]["schedule"]
    steps = workflow["jobs"]["publish"]["steps"]

    assert [entry["cron"] for entry in schedule] == ["7,37 * * * *"]
    assert workflow["concurrency"]["cancel-in-progress"] == "false"

    publish_step = next(step for step in steps if step.get("name") == "Publish Newsdesk")

    assert publish_step["run"] == ".venv/bin/python src/public_publish.py"
    assert not any(step.get("name") == "Repair stale Newsdesk publish" for step in steps)
